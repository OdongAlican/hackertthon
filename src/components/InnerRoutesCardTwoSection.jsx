...
const InnerRoutesCardTwoSection = ({ element, clickEvent }) => {
  ...
  const displayProduct = product => clickEvent(product);
  const deleteAction = () => {
    dispatch(deleteProduct(element.id));
  };
  const editAction = () => console.log(element, 'CRUD Modal');
    ...
    <div className="inner-routes-card-two-innner-section">
      ...
      <div className="inner-routes-card-two-right">
        <div className="inner-routes-card-two-right-first">
          <Button RequestButton buttonName="VIEW DETAILS" clickButton={() => displayProduct(element)} />
        </div>
        <div className="inner-routes-card-two-right-second">
          <Button RequestButtonAccepted buttonName="PURCHASE" />
        </div>
        <EditDeleteModal deleteAction={deleteAction} editAction={editAction} />
      </div>
    </div>
  );
};
